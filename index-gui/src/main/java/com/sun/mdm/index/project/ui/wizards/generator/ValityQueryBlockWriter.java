 ...
 */
class ValityQueryBlockWriter implements GenInterface {
    private static final String TEMPLATE_NAME = "ValityQueryBlockDefinition.tmpl";

    /**
     * Creates an instance
     */
    public ValityQueryBlockWriter() {
    }
        ...
        if (fieldSettings != null) {
            ...
            if (fieldSettings.getBlockOn()) {
                ...
                if (templateWriter != null) {
                    ...
                    String[] fields = new String[]{undecoratedQualifiedNodeName + "." + fieldName};
                    String genFieldPrefix = ConfigGenerator.getGenFieldPrefix(fieldSettings);
                    String blockDefinitionID = getID(configSettings, fieldSettings);
     ...
     * @return the id
     */
    private String getID(ConfigSettings configSettings, FieldSettings fieldSettings) {
        FieldSettings[] fields = configSettings.getFieldSettings();
        int blockFieldCount = 0;
        String id = null;
        for (int fieldCount = 0; id == null && fieldCount < fields.length; fieldCount++) {
            if (fields[fieldCount].equals(fieldSettings)) {
                id = "ID" + blockFieldCount;
            }
            if (fields[fieldCount].getBlockOn()) {
                blockFieldCount++;
            }
        }
        return id;
