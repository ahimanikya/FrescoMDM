...
public class ScriptGenerator {
	...
	private void generateFiles() {
		for (Table t : tables) {
			ControlFileWriter cfw = new ControlFileWriter(t, control
					.getAbsolutePath());
			cfw.write();
		...
		DropSQLWriter dsw = new DropSQLWriter(objectDefTables, sqlldr
				.getAbsolutePath());
		dsw.write();

		CreateSQLWriter csw = new CreateSQLWriter(objectDefTables, sqlldr
				.getAbsolutePath());
		csw.write();
		
		TruncateSQLWriter tsw = new TruncateSQLWriter(tables, sqlldr
				.getAbsolutePath());
		tsw.write();
		

		BatchFileWriter bfw = new BatchFileWriter(tables, sqlldr
				.getAbsolutePath());
		bfw.write();
	}
