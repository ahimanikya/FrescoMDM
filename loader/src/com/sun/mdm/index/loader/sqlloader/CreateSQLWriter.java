...
public class CreateSQLWriter extends DropSQLWriter {
	...
	protected void writeSQLForVariableTables(StringBuilder sb) {
		...
		for (int i = 0; i < tables.size() ; i++) {
			...
			String tableNameWithoutPrefix = tableName.substring("SBYN_"
					.length());

			String fk_contraint_name = null;

			if (tableName.equals(parent.getName())
					|| tableName.equals(parentSBR.getName())) {
				if (tableName.equals(parent.getName())) {
					fk_contraint_name = "FK_" + tableNameWithoutPrefix
							+ "_SYSTEMCODE_LID;";
				} else {// i==1
					fk_contraint_name = "FK_" + tableNameWithoutPrefix
							+ "_EUID;";
				...
				fk_contraint_name = "FK_" + tableNameWithoutPrefix + "_"
						+ parent_pk + ";";
			}

			String fk_contraint = "ALTER TABLE " + tableName
					+ getConstraintAction() + fk_contraint_name;
			String pk_contraint = "ALTER TABLE " + tableName
					+ getConstraintAction() + "PK_" + tableNameWithoutPrefix
					+ ";";
			String u_contraint = "ALTER TABLE " + tableName
					+ getConstraintAction() + "U_" + tableNameWithoutPrefix
					+ ";";
	...
	private static String createIndex = "CREATE INDEX SBYN_ENTERPRISE1 on SBYN_ENTERPRISE (EUID ASC);\n"
