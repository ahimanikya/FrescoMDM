 ...
 * 
 */
public class OracleBucketDAO extends BaseBucketDAO  {
	
	
	protected static String bucket_insert = "insert into cluster_bucket (id , bucketName, loaderName, type,state,versionno) values (bucket_id_seq.nextval,?,?,?,?,?)";

	protected static String bucket_select = "select id, bucketName, loaderName, type,state,versionno from cluster_bucket where state=? AND type=? AND ROWNUM <= 5";


	
	private static Logger logger = Logger.getLogger(OracleBucketDAO.class
			.getName());
