...
/** Test class for an alpha search using an Oracle wild card
 * @author Raymond Tam
 */
public class WildCardSearchEnterpriseObject extends TestCase {
   ...
   /** master controller instance
     ...
     */
    public WildCardSearchEnterpriseObject(String name) {
        super(name);
    }
    ...
    /** Tests alpha search with an Oracle wild card character
     * @throws Exception an error occured
     */
    public void testWildCardSearchEnterpriseObject() throws Exception {

        String defaultSearchType = "ALPHA-SEARCH";
        String defaultFirstName = "d%h";
        String defaultLastName = "wo%";
        String defaultSystem = "SBYN";
        String defaultLid = "4568915615";
        float defaultMinComparisonThreshold = -100;
    ...
    public static boolean run(String tmpSearchType, 
        ...
        System.out.println("\n---WildCardSearchEnterpriseObject---\n");
        ...
        // create search criteria (first name only)
        ...
        fields.add("Enterprise.SystemSBR.Person.StdFirstName");
        fields.add("Enterprise.SystemSBR.Person.StdLastName");
        ...
        while (r.hasNext()) {
            ...
            if (rec.getComparisonScore() >= minComparisonThreshold)  {
                ...
                System.out.println(rec + ": " + p.getFirstName() + " " +  p.getLastName() 
                        + " [" + p.getStdFirstName() + " " +  p.getStdLastName() + "], " + p.getSSN());
                count++;
                if (p.getLastName().compareTo("Womack") != 0)  {
                    System.out.println("Test failed: expected last name to be \"Womack\" " 
                            + "but retrieved \"" + p.getLastName() + "\"");
                    System.out.println("Expected " + expectedCount + " record.  Found " + count + " record(s)\n");
                    return false;
                }
                if (p.getFirstName().compareTo("Deb") != 0)  {
                    System.out.println("Test failed: expected first name to be \"Deb\" " 
                            + "but retrieved \"" + p.getFirstName() + "\"");
                    System.out.println("Expected " + expectedCount + " record.  Found " + count + " record(s)\n");
                    return false;
                }
            }
    ...
    public static void main(String[] args) {
        junit.textui.TestRunner.run(new TestSuite(WildCardSearchEnterpriseObject.class));
    }
