...
/**  Adds records for testing MCTransferSystemObject.
 *
 * @author  rtam
 * @version 
 */

public class AddRecordHelper2 extends BasicHelper {
    
    // Used for EO creation order--testMCTransferSystem1
    public static final String TRANSFER_TEST_ORDER0 = "FirstTransferTest";
    
    // Used for EO creation order--testMCTransferSystem2
    public static final String TRANSFER_TEST_ORDER1 = "SecondTransferTest";
    
    // Used for EO creation order--testMCTransferSystem3
    public static final String TRANSFER_TEST_ORDER2 = "ThirdTransferTest";
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            run(tmpSystem, tmpLid0, tmpLid1, tmpLid2, TRANSFER_TEST_ORDER2);
        }  catch (Exception e) {
            e.printStackTrace();
            System.exit(-1);
        }
    ...
    public static boolean run(String tmpSystem, 
        ...
        if (createOrder.equalsIgnoreCase(TRANSFER_TEST_ORDER1)) {
            sysObj = buildUnitTestSystemObject1();
        } else if (createOrder.equalsIgnoreCase(TRANSFER_TEST_ORDER0) 
                   || createOrder.equalsIgnoreCase(TRANSFER_TEST_ORDER2)){
            sysObj = buildUnitTestSystemObject0();
        } else {
            System.out.println("Error.  Unrecognized transfer test order: " + createOrder);
            return false;
        }
        MatchResult mr = controller.executeMatch(sysObj);
        euid1 = mr.getEUID();
        EnterpriseObject eo = controller.getEnterpriseObject(euid1);
        ...
        if (createOrder.equalsIgnoreCase(TRANSFER_TEST_ORDER0)) {
            // do nothing
        } else if (createOrder.equalsIgnoreCase(TRANSFER_TEST_ORDER1)) {
            sysObj = buildUnitTestSystemObject0();
            eo.addSystemObject(sysObj);        
            controller.updateEnterpriseObject(eo);
        } else if (createOrder.equalsIgnoreCase(TRANSFER_TEST_ORDER2)){
            sysObj = buildUnitTestSystemObject1();
            eo.addSystemObject(sysObj);        
            controller.updateEnterpriseObject(eo);
        } else {  // do nothing
