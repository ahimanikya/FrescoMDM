 ...
 * @author jlong
 */
public class ClientSetBooleanField extends TestCase {
    /**
     * constructor
     * @param name name
     */
    public ClientSetBooleanField (String name) {
        super(name);
    }
     ...
     */
    public void testClientSetBooleanField() throws Exception {
        ...
        java.sql.PreparedStatement ps = ops.getStatement("update SBYN_ALIAS set MYBOOLEANFIELD = ? where ALIASID = '" + alias.getAliasId() + "'", conn);
        ops.setParam(ps, 1, "Boolean", new Boolean(true));
        int status = ps.executeUpdate();
        System.out.println("Update status: " + status);    
        assertTrue(status == 1);
        ps.close();
        
        ps = ops.getStatement("select MYBOOLEANFIELD from SBYN_ALIAS where ALIASID = '" + alias.getAliasId() + "'", conn);
        ResultSet rs = ps.executeQuery();
        rs.next();
        conn.commit();
        Boolean b = (Boolean) ops.getValue(rs, "MYBOOLEANFIELD", "Boolean");
        System.out.println("MYBOOLEANFIELD = " + b);  
