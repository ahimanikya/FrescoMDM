 ...
 * @author Dan Cidon
 */
public class ExecuteMatchDeferredHelper extends BasicHelper {
    
    private boolean performPessimisticMode;
    /** Constructor
     */
    public ExecuteMatchDeferredHelper(boolean performPessimisticMode) { 
        this.performPessimisticMode = performPessimisticMode;
    }
     ...
     */
    public List runDupRecalc(String args[]) throws Exception {
        setArgs(args);
        ...
        while (record != null) {
            ...
            Boolean val = new Boolean(performPessimisticMode);
            MatchResult mr = mc.executeMatchDupRecalc(sysobj[0], val);
            eoList.add(mr);
            if (sysobj.length > 1) {
    ...
    public List runUpdateDupRecalc(String args[]) throws Exception {
        setArgs(args);
        if (getBooleanValue("clearDb")) {
            ClearDb.run();
        }
