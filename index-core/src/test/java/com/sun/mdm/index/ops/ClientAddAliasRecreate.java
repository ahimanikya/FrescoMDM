 ...
 * @author gzheng
 */
public class ClientAddAliasRecreate extends TestCase {
    /**
     * constructor
     * @param name name
     */
    public ClientAddAliasRecreate(String name) {
        super(name);
    }
     ...
     */
    public void testClientAddAliasRecreate() throws Exception {
        TestUtils test = new TestUtils();
        TransactionMgr tm = test.getTransactionMgr();
        Connection conn = test.getConnection();
        
        String tid = test.getTMID(test.ADDALIAS);
        ...
        for (int i = 0; i < aliases.size(); i++) {
            ...
            if (a.getFirstName().equals("Gary") && a.getLastName().equals("Zheng") && a.getMiddleName().equals("G")) {
                found = true;
        ...
        if (aliases != null) {
            for (int i = 0; i < aliases.size(); i++) {
                AliasObject a = (AliasObject) aliases.get(i);
                if (a.getFirstName().equals("Gary") 
                    && a.getLastName().equals("Zheng") && a.getMiddleName().equals("G")) {
                    found = true;
