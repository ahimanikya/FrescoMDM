...
public class MasterControllerCoreImpl implements MasterControllerCore {
	...
	public void init(SessionContext context) throws Exception {
		try {
			...
			if (mLogger.isDebugEnabled()) {
				mLogger.debug("Creating UpdateManager");
			}

			mUpdate = UpdateManagerFactory.getInstance();

			if (mLogger.isDebugEnabled()) {
				mLogger.debug("UpdateManager created");
				mLogger.debug("Creating QueryManager");
			}

			mQuery = QueryManagerFactory.getInstance();
			if (mLogger.isDebugEnabled()) {
				mLogger.debug("QueryManager created");
				mLogger.debug("Creating Match Engine Controller");
			}

			mMatch = MatchEngineControllerFactory.getInstance();

			if (mLogger.isDebugEnabled()) {
				mLogger.debug("Match Engine Controller created");
				mLogger.debug("Creating Survivor Calculator");
			}
			mCalculator = new com.sun.mdm.index.survivor.SurvivorCalculator(
					mMatch);
			if (mLogger.isDebugEnabled()) {
				mLogger.debug("Survivor Calculator created");
				mLogger.debug("Creating Transaction Manager");
			}
			mTrans = TransactionMgrFactory.getInstance();

			if (mLogger.isDebugEnabled()) {
				mLogger.debug("Transaction Manager created");
			}
			...
			if (mLogger.isDebugEnabled()) {
				mLogger.debug("Reading Master Controller Configuration");
			}
			...
			mLogger.info("Execute match search options: " + mSearchOptions);
			mLogger.info("Pessimistic mode: " + pessimisticModeEnabled);
			mLogger.info("Merge record update: " + mMergedRecordUpdateEnabled);
			...
			if (classString == null || classString.length() == 0) {
				...
				mLogger.info("Create user logic class: " + classString);
			...
			if (classString == null || classString.length() == 0) {
				...
				mLogger.info("Create user logic class for GUI: " + classString);
				Object logicInstance = Class.forName(classString).newInstance();
				if (logicInstance instanceof com.sun.mdm.index.master.ExecuteMatchLogics) {
			...
			if (mLogger.isDebugEnabled()) {
				mLogger.debug("Setting up outbound sender");
			}
			mOutBoundSender = setupOutBoundSender();
			if (mLogger.isDebugEnabled()) {
				mLogger.debug("Outbound sender setup complete");
			}

			// Determine decision maker
			if (mLogger.isDebugEnabled()) {
				mLogger.debug("Setting up decision maker");
			}
			...
			mLogger.info("Decision maker class: " + mDecision.getClass());
			mLogger.info("Potential duplicate threshold: "
					+ mDuplicateThreshold);

			if (mLogger.isDebugEnabled()) {
				mLogger.debug("initialization done");
			}
	...
	public String getEUID(Connection con, SystemObjectPK key)
		...
		try {
			mLogger.info("getEuid(): looking up system key: [" + key.lID + ", "
					+ key.systemCode + "]");
			...
			mLogger.info("getEuid(): found euid: " + euid);
	...
	public EnterpriseObject getEnterpriseObject(Connection con, String euid)
		...
		try {
			mLogger.info("getEnterpriseObject(): looking up EUID: " + euid);
	...
	public EnterpriseObject getEnterpriseObject(Connection con,
		...
		try {
			...
			mLogger
					.info("getEnterpriseObject(): looking up system key: "
							+ key);
	...
	public MergeHistoryNode getMergeHistory(Connection con, String euid)
		...
		try {
			mLogger.info("getMergeHistory(): looking up EUID: " + euid);
	...
	public SBR getSBR(Connection con, String euid) throws ProcessingException,
		...
		try {
			mLogger.info("getSBR(): looking up EUID: " + euid);
	...
	public SystemObject getSystemObject(Connection con, SystemObjectPK key)
		...
		try {
			mLogger.info("getSystemObject(): looking up system key: " + key);
	...
	public void activateSystemObject(Connection con, SystemObjectPK systemKey)
		...
		try {
			mLogger.info("activateSystemObject(): looking up system key: "
					+ systemKey);
			...
			SystemObject so = eo.getSystemObject(systemKey.systemCode,
					systemKey.lID);
			mLogger.info("activateSystemObject(): setting status of "
					+ systemKey + " to " + SystemObject.STATUS_ACTIVE);
			so.setValue("Status", SystemObject.STATUS_ACTIVE);
			mLogger.info("activateSystemObject(): updating EUID: " + euid);
			...
			if ((sbr.getStatus() != null)
				...
				mLogger
						.info("activateSystemObject(): execute pessimistic for EUID: "
								+ eo2.getEUID());
	...
	public void activateEnterpriseObject(Connection con, String euid)
		...
		try {
			mLogger
					.info("activateEnterpriseObject(): activating EUID: "
							+ euid);
	...
	public void addSystemObject(Connection con, String euid, SystemObject sysObj)
		...
		try {
			...
			mLogger.info("addSystemObject(): adding system key " + systemKey
					+ " to EUID: " + euid);
			...
			mLogger.info("addSystemObject(): adding: " + systemKey
					+ " to EUID: " + euid);
	...
	public SBR calculateSBR(EnterpriseObject eo) throws ProcessingException,
		...
		SBR sbr = null;
		try {
			mLogger.info("calculateSBR(): invoked for EUID: " + eo.getEUID());
	...
	public EnterpriseObject createEnterpriseObject(Connection con,
		...
		try {
			SystemObjectPK systemKey = new SystemObjectPK(sysobj
					.getSystemCode(), sysobj.getLID());
			mLogger
					.info("createEnterpriseObject(): creating EO for system key: "
							+ systemKey);
			sysobj = mMatch.standardize(sysobj);
			validateSystemObject(con, sysobj);
	...
	public EnterpriseObject createEnterpriseObject(Connection con,
		...
		try {
			for (int i = 0; i < sysobj.length; i++) {
				mLogger
						.info("createEnterpriseObject(): processing system key ["
	...
	public void deactivateSystemObject(Connection con, SystemObjectPK systemKey)
		...
		try {
			mLogger.info("deativateSystemObject(): invoked for system key: "
					+ systemKey);
			...
			SystemObject so = eo.getSystemObject(systemKey.systemCode,
					systemKey.lID);
			mLogger.info("deactivateSystemObject() setting status of key "
			...
			if ((afterEO.getSBR().getStatus() != null)
					&& (!afterEO.getSBR().getStatus().equals(
							SystemObject.STATUS_ACTIVE))) {
				mLogger
						.info("deactivateSystemObject(): removing potential duplicates for non-active EUID: "
								+ afterEO.getEUID());
	...
	public void deactivateEnterpriseObject(Connection con, String euid)
		...
		try {
			mLogger.info("deactivateEnterpriseObject(): invoked for EUID: "
					+ euid);
			...
			if ((afterEO.getSBR().getStatus() != null)
					&& (!afterEO.getSBR().getStatus().equals(
							SystemObject.STATUS_ACTIVE))) {
				mLogger
						.info("deactivateEnterpriseObject(): removing potential duplicates for non-active EUID: "
								+ afterEO.getEUID());
				mPotDup.deletePotentialDuplicates(con, afterEO.getEUID());
			}
	...
	public void deleteSystemObject(Connection con, SystemObjectPK systemKey)
		...
		try {
			mLogger.info("deleteSystemObject(): invoked for system key "
					+ systemKey);
				...
				if (afterEO.isRemoved()
						|| ((afterEO.getSBR().getStatus() != null) && (!afterEO
								.getSBR().getStatus().equals(
										SystemObject.STATUS_ACTIVE)))) {
					mLogger
							.info("deleteSystemObject(): removing potential duplicates for non-active EUID: "
									+ afterEO.getEUID());
					mPotDup.deletePotentialDuplicates(con, afterEO.getEUID());
				} else {
					if (pessimisticModeEnabled) {
	...
	private MatchResult processMatch(Connection con, SystemObject sysObj,
		...
		SystemObjectPK systemKey = new SystemObjectPK(sysObj.getSystemCode(),
				sysObj.getLID());
		mLogger.info("processMatch(): invoked for system key: " + systemKey);
		try {
			sysObj = mMatch.standardize(sysObj);
		...
		try {
			...
			mLogger.info("processMatch(): looking up system key " + systemKey);
			euid = mQueryHelper.getEUID(con, systemKey);
			if (euid != null) {

				if (userLogics.disallowUpdate(sysObj)) {
					if (mLogger.isDebugEnabled()) {
						mLogger.debug("processMatch(): Update disallowed: "
								+ euid);
					}
					resultCode = MatchResult.UPDATE_NOT_ALLOWED;
				} else {
					mLogger.info("processMatch(): loading EUID " + euid);
					...
					if (userLogics.rejectUpdate(sysObj, beforeEO)) {
						if (mLogger.isDebugEnabled()) {
							mLogger.debug("processMatch(): Update rejected: "
									+ euid);
						}
						...
						Object[] beforeMatchFields = mMatchFieldChange
								.getMatchFields(beforeEO);
						mLogger.info("processMatch(): updating EUID " + euid);
				...
				if (userLogics.bypassMatching(sysObj)) {
					if (mLogger.isDebugEnabled()) {
						mLogger.debug("processMatch(): Matching bypassed");
					}
					if (userLogics.disallowAdd(sysObj)) {
						if (mLogger.isDebugEnabled()) {
							mLogger.debug("processMatch(): Add disallowed");
						}
					...
					mLogger.info("processMatch(): system key " + systemKey
					...
					list = mMatch.findMatch(con, crit, mSearchOptions,
							matchOpts);
					mLogger.info("processMatch(): match query completed");

					if (list != null && list.size() > 0) {
						mLogger
								.info("processMatch(): records found, invoking decision maker");
						dmr = mDecision.process(con, list, sysObj);
						assumedMatch = dmr.getAssumedMatch();
					} else {
						mLogger.info("processMatch(): no records found");
					}

					if (assumedMatch == null) {
						mLogger
								.info("processMatch(): no EO matched, creating new EO");
						if (userLogics.disallowAdd(sysObj)) {
							if (mLogger.isDebugEnabled()) {
								mLogger.debug("processMatch(): Add disallowed");
							}
						...
						mLogger
								.info("processMatch(): assumed match, updating EUID "
										+ euid);
						...
						if (userLogics.rejectAssumedMatch(sysObj, beforeEO)) {
							if (mLogger.isDebugEnabled()) {
								mLogger
										.debug("processMatch(): Assumed match rejected");
							}
							// new Enterprise object
							if (userLogics.disallowAdd(sysObj)) {
								if (mLogger.isDebugEnabled()) {
									mLogger
											.debug("processMatch(): Add disallowed");
								}
								resultCode = MatchResult.ADD_NOT_ALLOWED;
							} else {
								createNewEO = true;
							...
							if (userLogics.disallowUpdate(sysObj)) {
								if (mLogger.isDebugEnabled()) {
									mLogger
											.debug("processMatch(): Update disallowed");
								}
								resultCode = MatchResult.UPDATE_NOT_ALLOWED;
							} else {
								Object[] beforeMatchFields = mMatchFieldChange
	...
	public void insertAuditLog(Connection con, AuditDataObject auditObject)
		...
		try {
			mLogger.info("insertAuditLog(): record " + auditObject);
	...
	public AssumedMatchIterator lookupAssumedMatches(Connection con,
		...
		try {
			mLogger
					.info("lookupAssumedMatches(): invoked with search object: ");
			mLogger.info(o);
			...
			if (o.getPageSize() == 0) {
				if (mLogger.isDebugEnabled()) {
					mLogger
							.debug("lookupAssumedMatches(): setting page size to default "
									+ defaultPageSize);
				}
			...
			if (maxElements == 0) {
				if (mLogger.isDebugEnabled()) {
					mLogger
							.debug("lookupAssumedMatches(): setting max elements to default "
									+ defaultMaxElements);
				}
	...
	public String undoAssumedMatch(Connection con, String assumedMatchId)
		...
		try {
			mLogger.info("undoAssumedMatch(): invoked for assumed match id: "
					+ assumedMatchId);
			...
			if (i.hasNext()) {
				...
				mLogger
						.info("undoAssumedMatch(): system key to undo assumed match ["
								+ sysCode
								+ ", "
								+ lid
								+ "], EUID: "
				...
				mLogger.info("undoAssumedMatch(): new EUID constructed: "
						+ newEuid);
			...
			mLogger.info("undoAssumedMatch(): deleting assumed match id: "
					+ assumedMatchId);
	...
	public AuditIterator lookupAuditLog(Connection con, AuditSearchObject obj)
		...
		try {
			mLogger.info("lookupAuditLog() invoked with search object: ");
			mLogger.info(obj);
	...
	public PotentialDuplicateIterator lookupPotentialDuplicates(Connection con,
		...
		try {
			mLogger
					.info("lookupPotentialDuplicates(): invoked with search object: ");
			mLogger.info(obj);
			...
			if (obj.getPageSize() == 0) {
				if (mLogger.isDebugEnabled()) {
					mLogger
							.debug("lookupPotentialDuplicates(): setting page size to default "
									+ defaultPageSize);
				}
			...
			if (maxElements == 0) {
				if (mLogger.isDebugEnabled()) {
					mLogger
							.debug("lookupPotentialDuplicates(): setting max elements to default "
									+ defaultMaxElements);
				}
	...
	public SystemDefinition[] lookupSystemDefinitions(Connection con)
		...
		try {
			mLogger.info("lookupSystemDefinitions(): invoked");

			sd = mQueryHelper.lookupSystemDefinitions(con);
			if (mLogger.isDebugEnabled()) {
				if (sd == null) {
					mLogger
							.debug("lookupSystemDefinitions(): no systems defined");
				} else {
					for (int i = 0; i < sd.length; i++) {
						mLogger.debug("lookupSystemDefinitions(): " + sd[i]);
					}
				}
			}
		} catch (ProcessingException e) {
	...
	public SystemDefinition lookupSystemDefinition(Connection con,
		...
		try {
			...
			if (mLogger.isDebugEnabled()) {
				if (sd == null) {
					mLogger.debug("lookupSystemDefinition() for: " + systemCode
							+ " no systems defined");
				}
			}
		} catch (ProcessingException e) {
	...
	public SystemObjectPK[] lookupSystemObjectPKs(Connection con, String euid)
		...
		try {
			mLogger.info("lookupSystemObjectPKs(): invoked with EUID: " + euid);

			retVal = mQueryHelper.lookupSystemObjectKeys(con, euid);
			if (mLogger.isDebugEnabled()) {
				if (retVal != null) {
					for (int i = 0; i < retVal.length; i++) {
						mLogger.debug("lookupSystemObjectPKs(): " + retVal[i]);
					}
				} else {
					mLogger
							.debug("lookupSystemObjectPKs(): "
		...
		try {
			...
			if (mLogger.isDebugEnabled()) {
				if (retVal != null) {
					...
					mLogger.debug("lookupSystemObjectPKs(): "
	...
	public SystemObjectPK[] lookupSystemObjectPKs(Connection con,
		...
		try {
			if (mLogger.isInfoEnabled()) {
				mLogger
						.info("lookupSystemObjectPKs(): invoked with source system code: "
								+ sourceSystem
								+ ", source LID: "
								+ sourceLID
	...
	public SystemObject[] lookupSystemObjects(Connection con, String euid)
		...
		try {
			mLogger.info("lookupSystemObjects(): invoked with EUID: " + euid);
	...
	public TransactionSummary lookupTransaction(Connection con, String transId)
		...
		try {
			mLogger.info("lookupTransaction(): invoked with transaction id: "
					+ transId);
	...
	public TransactionIterator lookupTransactions(Connection con,
		...
		try {
			mLogger.info("lookupTransactions(): invoked with search object: ");
			mLogger.info(searchObj);
			...
			if (arTrans != null) {
				...
				if (searchObj.getPageSize() == 0) {
					if (mLogger.isDebugEnabled()) {
						mLogger
								.debug("lookupTransactions(): setting page size to default "
										+ defaultPageSize);
					}
	...
	public MergeResult mergeEnterpriseObject(Connection con, String sourceEUID,
		...
		MergeResult mr = null;
		try {
			mLogger.info("mergeEnterpriseObject(): invoked with source EUID: "
	...
	public MergeResult mergeEnterpriseObject(Connection con, String sourceEUID,
		...
		if (mLogger.isDebugEnabled()) {
			mLogger
					.debug("mergeEnterpriseObject(): updated destination image:");
			mLogger.debug(destinationEO);
		}
	...
	public MergeResult mergeSystemObject(Connection con, String systemCode,
		...
		mLogger.info("mergeSystemObject(): invoked with source system key: "
	...
	public MergeResult mergeSystemObject(Connection con, String systemCode,
		...
		if (mLogger.isDebugEnabled()) {
			mLogger.debug("mergeSystemObject(): updated destination image:");
			mLogger.debug(destImage);
		}
	...
	public void resolvePotentialDuplicate(Connection con, String id,
		...
		mLogger.info("resolvePotentialDuplicate(): invoked with id: " + id
	...
	public EOSearchResultIterator searchEnterpriseObject(
		...
		mLogger
				.info("searchEnterpriseObject(): invoked (set log to DEBUG for details)");

		if (mLogger.isDebugEnabled()) {
			mLogger.debug("searchEnterpriseObject(): search criteria:");
			mLogger.debug(criteria);
			mLogger.debug("searchEnterpriseObject(): search options:");
			mLogger.debug(searchOptions);
		}
		...
		try {
			...
			if (maxElements == 0) {
				if (mLogger.isDebugEnabled()) {
					mLogger
							.debug("searchEnterpriseObject(): setting max elements to default "
									+ maxConfigElements);
				}
			...
			if (mLogger.isDebugEnabled()) {
				mLogger.debug("searchEnterpriseObject(): standardize: "
						+ queryBuilder.isStandardizeRequired());
				mLogger.debug("searchEnterpriseObject(): phoneticize: "
			...
			if (searchOptions.isWeighted()) {
				...
				if (mLogger.isDebugEnabled()) {
					mLogger.debug("searchEnterpriseObject(): "
	...
	public EOSearchResultIterator searchEnterpriseObject(String Euids[],
		...
		if (mLogger.isDebugEnabled()) {
			...
			mLogger.debug("searchEnterpriseObject(): EUIDs to retrieve:"
					+ sb.toString());
		}
	...
	public void transferSystemObject(Connection con, String destinationEUID,
		...
		try {
			mLogger
					.info("transferSystemObject(): invoked with destination EUID: "
				...
				if (afterSrcEO.isRemoved()
						|| (afterSrcEO.getSBR().getStatus() != null && !afterSrcEO
								.getSBR().getStatus().equals(
										SystemObject.STATUS_ACTIVE))) {
					mLogger
							.info("transferSystemObject(): removing potential duplicates for non-active EUID: "
									+ afterSrcEO.getEUID());
					mPotDup
							.deletePotentialDuplicates(con, afterSrcEO
									.getEUID());
				} else {
					if (pessimisticModeEnabled) {
						String transId = result.getTransactionResult()
	...
	public MergeResult unmergeEnterpriseObject(Connection con,
		...
		mLogger.info("unmergeEnterpriseObject(): invoked with EUID: "
		...
		try {
			...
			if (mLogger.isDebugEnabled()) {
				mLogger
						.debug("unmergeEnterpriseObject(): Unmerging transaction ["
								+ transId + "]");
			}
	...
	public MergeResult unmergeSystemObject(Connection con, String systemCode,
		...
		SystemObjectPK sourceSystemKey = new SystemObjectPK(systemCode,
				sourceLID);
		SystemObjectPK destSystemKey = new SystemObjectPK(systemCode, destLID);
		mLogger.info("unmergeSystemObject(): invoked with source system key: "
				+ sourceSystemKey + ", destination system key: "
		...
		try {
			...
			if (mLogger.isDebugEnabled()) {
				mLogger
						.debug("unmergeSystemObject(): searching for unmerge transaction");
				mLogger.debug(tObj);
			}
			...
			if (mLogger.isDebugEnabled()) {
				mLogger.debug("unmergeSystemObject(): undoing transaction: "
						+ transObj.getTransactionNumber());
				mLogger.debug(transObj);
			}
			...
			if (!calculateOnly) {
				...
				if (sourceEO != null) {
					...
					if (activeCount > 1) {
						...
						if ((sourceEO.getSBR().getStatus() != null)
							...
							mLogger
									.info("unmergeSystemObject(): find and insert potential duplicates for activated EUID: "
											+ sourceEO.getEUID());
							findInsertDuplicates(con, sourceEO.getEUID(),
									transId, sourceSBR, null);
						}
				...
				if (mLogger.isDebugEnabled()) {
					mLogger
							.debug("unmergeSystemObject(): unmerge transaction completed, trans id: "
									+ transId);
				}
	...
	public void unresolvePotentialDuplicate(Connection con, String id)
		...
		try {
			mLogger.info("unresolvePotentialDuplicate(): invoked with id: "
					+ id);
	...
	public void updateEnterpriseObject(Connection con, EnterpriseObject eo)
			throws ProcessingException, UserException {
		mLogger.info("updateEnterpriseObject() invoked for EUID: "
				+ eo.getEUID());
	...
	public UpdateResult updateEnterpriseDupRecalc(Connection con,
			...
			throws ProcessingException, UserException {
		mLogger.info("updateEnterpriseDupRecalc() invoked for EUID: "
				+ eo.getEUID());
		Object[] matchFields = null;
		try {
			Iterator i = eo.getSystemObjects().iterator();
	...
	private void updateSystemObjectFacade(Connection con, SystemObject sysobj,
		...
		SystemObjectPK key = new SystemObjectPK(sysobj.getSystemCode(), sysobj
				.getLID());
		if (mLogger.isDebugEnabled()) {
			mLogger.debug("updateSystemObject(): invoked for system key: "
	...
	public Object getConfigurationValue(String param)
		...
		if (param.equals("EUID_LENGTH")) {
			try {
				...
				mLogger.error("getConfigurationValue(): error", e);
				throwProcessingException(e);
			}
		} else {
	...
	public Integer getRevisionNumber(Connection con, String euid)
		...
		try {
			mLogger.info("getRevisionNumber(): looking up EUID: " + euid);
	...
	private OutBoundSender setupOutBoundSender() throws ProcessingException {
		...
		if (is != null) {
			...
			mLogger.info("setupOutBoundSender(): connection factory name: "
					+ factoryName);
			mLogger.info("setupOutBoundSender(): topic name: " + topicName);
			sender.initialize(factoryName, topicName);
			mLogger.info("setupOutBoundSender(): Outbound message enabled");
		} else {
			mLogger.info("setupOutBoundSender(): " + TOPIC_PROP_FILE
	...
	public void calculatePotentialDuplicates(Connection con, String euid,
		...
		mLogger.info("calculatePotentialDuplicates(): invoked with EUID: "
				+ euid);
	...
	private MergeResult mergeSystemObject(Connection con,
		...
		if (fromEUID.equals(toEUID)) {
			mLogger.info("mergeSystemObject(): both system objects in EUID: "
					+ toEUID);
			...
			mLogger.info("mergeSystemObject(): source system object in EUID: "
			...
			if (!calculateOnly) {
				...
				if (afterSourceEO.isRemoved()) {
					mLogger
							.info("mergeSystemObject(): deleting potential duplicates "
	...
	public void releaseResources(Connection con) {
		try {
			...
			mLogger.error(
					"releaseConnection(): could not close JDBC connection", e);
		} catch (OutBoundException e) {
			mLogger.error(
					"releaseConnection(): could not release JMS resources", e);
		}
	}
	...
	private void rollback(Connection con) {
		try {
			...
			mLogger.error("rollback(): could not rollback transaction", e);
		}
	}
	...
	private void commit(Connection con) {
		try {
			...
			mLogger.error("commit(): could not commit transaction", e);
		}
	}
	...
	private void validateSystemObject(Connection con, SystemObject sysObj)
		...
		if (Constants.VALIDATION) {
			...
			if (mLogger.isDebugEnabled()) {
				mLogger
						.debug("validateSystemObject(): validating system object ["
								+ sysCode + ", " + lid + "]");
			}
			...
			if (lidValidator != null) {
				try {
					mLogger.info("LIdValidator:" + lidValidator);
					lidValidator.validate(sysObj);
				} catch (ValidationException v) {
	...
	throws ProcessingException, UserException {
		if (Constants.VALIDATION) {
			try {
				if (mLogger.isDebugEnabled()) {
					mLogger
							.debug("validateEnterpriseObject(): validating EUID "
									+ entObj.getEUID());
				}
			} catch (Exception e) {
				throw new ProcessingException(e);
	...
	private synchronized void setMBeanServer() {
		...
		if (mMBeanServer == null) {
			mLogger.error("Could not find mBeanServer");
		}
	...
	private void sendAlert(String message) {
		try {
			...
			mLogger.error("sendAlert(): error sending message", ex);
		}
	}
	...
	private void sendCriticalError(String message) {
		try {
			...
			mLogger.error("sendCriticalError(): error sending message", ex);
		}
	}
	...
	private String getMBeanObjectNameStr() throws ProcessingException {
		...
		if (is != null) {
			...
			if (mLogger.isDebugEnabled()) {
				mLogger
						.debug("AppMBean object name is : "
								+ mBeanObjectNameStr);
			}
		} else {
			if (mLogger.isDebugEnabled()) {
				mLogger.info("getMBeanObjectNameStr(): " + MBEAN_PROP_FILE
						+ "not found");
			}
		}
