 ...
 */
class CodeValueType {
    private final String mFmat = "                    [elephantCodeValueType]";
        ...
        if (node.hasChildNodes()) {
            ...
            for (int i = 0; i < nl.getLength(); i++) {
                if (nl.item(i).getNodeType() == Node.ELEMENT_NODE) {
                    if (mTagValue.equals(((Element) nl.item(i)).getTagName())) {
                        strValue = Utils.getStrElementValue(nl.item(i));
                    }
                    if (mTagDescription.equals(
