...
<tr>
<% int colsspan = transactionHandler.getScreenConfigArray().size();
if(transactionHandler.getParametersMap().get("selectedSearchType") != null) {
  colsspan = colsspan+1;
}%>
...
<% //Build the request Map 
   HashMap keyDescriptionsMap = transactionHandler.getKeyDescriptionsMap();
...
<%if(transactionHandler.getParametersMap().get("selectedSearchType") != null) {%>
<td>
	  <span><%=bundle.getString("search_Type")%>:&nbsp;<b><%=transactionHandler.getParametersMap().get("selectedSearchType")%></b></span>
</td>
<%}%>

<%
  ArrayList screenConfigArrayLocal = transactionHandler.getScreenConfigArray();
%>
 <%
