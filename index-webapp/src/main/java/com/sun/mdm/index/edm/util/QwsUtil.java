...
public final class QwsUtil {
    private static final Logger mLogger = LogUtil.getLogger("com.sun.mdm.index.edm.util.QwsUtil");
     ...
     *      object. E.g., "Person".
     * @param fieldPath the field of interest. E.g., for major object, it's the
     *      field directly. For minor object, it's in the format of
     *      "Address.City".
    ...
    public static Collection getValueForField(ObjectNode majorNode,
        ...
        if (objRef == null) {
            // dealing major object.
            valueList.add(getValueFromNode(majorNode, fieldPath, user, isObjSensitive));
        } else {
            // dealing with minor object fields.
            ArrayList childNodes = majorNode.pGetChildren(objRef);
            String fieldName = getFieldNameFromFieldPath(fieldPath);

            if (childNodes != null) {
                for (Iterator i = childNodes.iterator(); i.hasNext();) {
                    valueList.add(getValueFromNode((ObjectNode) i.next(),
    ...
    public static String getLastObjRefFromFieldPath(String fieldPath) {
        ...
        int endIndex = fieldPath.lastIndexOf(ConfigManager.FIELD_DELIM);
        int startIndex = fieldPath.lastIndexOf(ConfigManager.FIELD_DELIM, endIndex - 1);

        if (startIndex > -1) {
            objRef = fieldPath.substring(startIndex + 1, endIndex);
        } else {
            objRef = fieldPath.substring(0, endIndex);
        }
    ...
    private static ResourceBundle getQwsResources() {
        try {
            ...
            mLogger.error("Error occurs while getting resource bundle: ", e);
        }
...
//	}
    ...
    public static void setObjectNodeFieldValue(ObjectNode node, String field, 
        ...
        if (valueString == null) {
           if (node.isNullable(field)) {
            ...
            return;
              ...
              throw new ValidationException("Field <" + fieldDisplayName + "> is required");
          }
        }
        ...
        try {
            ...
            //mLogger.error("Error in invalid value <" + valueString + "> for field " + field, e);
            ObjectNodeConfig config = ConfigManager.getInstance()
                                               .getObjectNodeConfig(node.pGetType());
            String fieldDisplayName = config.getFieldConfig(field).getDisplayName(); 
            throw new ValidationException("Invalid value <" + valueString + "> for field " + fieldDisplayName);
        }
    }
    ...
    public static Object getObjectValue(String valueString, int type)
        ...
        try {
            ...
            mLogger.error("Error in invalid value <" + valueString + "> for field " + type, e);
            throw new ObjectException("Invalid value string <" + valueString + "> for field type " + type);
        }
    }
    ...
    public static boolean isObjectNodeSensitive(ObjectNode obj) throws ObjectException {
      ...
      try {
          ...
          mLogger.error("Failed to detect sensitivity of object node", ex);
