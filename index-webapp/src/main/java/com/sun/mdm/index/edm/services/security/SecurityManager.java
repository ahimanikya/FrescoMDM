...
public class SecurityManager {
    ...
    private static final Logger mLogger 
        = LogUtil.getLogger("com.sun.mdm.index.edm.service.security.SecurityManager");
    ...
    public static void logon(String userid, String password) 
        ...
        if (userid == null) {
            throw new UserException("Error: User ID is may not be null");
        }
        if (userid.length() == 0) {
            throw new UserException("Error: User ID may not be an empty string");
        }
        if (password == null) {
            throw new UserException("Error: Password is may not be null");
        }
        if (password.length() == 0) {
            throw new UserException("Error: Password may not be an empty string");
        }
        try {
            Logon.execute(userid, password);
        } catch (Exception e) {
            mLogger.error("Login unsuccessful.");
    ...
    public static SecurityManager init() throws Exception {
        ...
        synchronized (SecurityManager.class) {
            ...
            try {
                ...
                mLogger.error("Failed to read Security permissions: ", e);
                instance = null;
                throw new Exception("Failed to read Security permissions: " + e.getMessage());
            }
            mLogger.info("Parsed Security permissions file successfully.");
        }
        return instance;
    }
    ...
    private void read() throws Exception {
        ...
        try {
            ...
            mLogger.error("Unable to open roles configuration file " + SecurityManager.ROLES_FILENAME, e);
            throw new Exception ("couldn't open file " + SecurityManager.ROLES_FILENAME);
        }
        read(in);
        in.close();
    }
            ...
            if (element.getTagName().equalsIgnoreCase(ROLES)) {
                ...
                while ( roleItr.hasNext() ) {
                    ...
                    try {
                      ...
                      mLogger.error("Error occurs in role definition: " + ex.getMessage());
                      throw(new Exception("Error in role definition: " + ex.getMessage()));
                    }
                }
            }
        }
    ...
    public ArrayList getOperationsforRole(String roleName) throws Exception {
        ...
        if (role != null) {
            ...
            throw new Exception("Error: role name not found [" + roleName + "]");
        }
    }
